...
def sentence_probability(sentence: str, ngram_model: list[dict], total_size):
    ...
    if len(sentence_tokens) != 0:
        ...
        # This needs future work : refactoring
        return None
   ...
   # ---------------------------------------------------------------------
    ...
    for i in textfile_list:
        ...
        with open(i, 'r', encoding = "utf8") as f:
            ...
            tokens.extend(preprocess(lines)) # it adds all the tokens to the list
    ...
    # goes through every element of the list of files to open the every file
    for i in textfile_list:
        # opens the file 
        with open(i, 'r', encoding = "utf8") as f:
            ...
            for sentence in sentences:
                if sentence != "":
                    bigram_probability = sentence_probability(sentence, bigram_model, total_size)
                    unigram_probability = sentence_probability(sentence, [unigrams], total_size) 

                    if bigram_probability != None:
                        average_bigram += bigram_probability
                        average_unigram += unigram_probability
                        probability_counter += 1
        
    
    average_bigram /= probability_counter
    average_unigram /= probability_counter

    print("AVERAGE LIKELIHOOD BIGRAM:", average_bigram,"\nAVERAGE LIKELIHOOD UNIGRAM:", average_unigram) 
